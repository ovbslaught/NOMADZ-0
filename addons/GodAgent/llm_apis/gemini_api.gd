...
const DEPRECATED_API_KEY_FILE := "res://addons/GodAgent/llm_apis/gemini_api_key.gd"
...
func send_chat_request(http_request: HTTPRequest, message_list: Array) -> bool:
	if not super.send_chat_request(http_request, message_list):
		return false
...
static func _deprecated_load_api_key_from_file() -> String:
