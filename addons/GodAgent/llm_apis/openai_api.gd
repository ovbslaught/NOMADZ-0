...
const DEPRECATED_API_KEY_SETTING := "plugins/ai_assistant_hub/openai_api_key"
const DEPRECATED_API_KEY_FILE := "res://addons/GodAgent/llm_apis/openai_api_key.gd"
...
func send_get_models_request(http_request: HTTPRequest) -> bool:
	...
	if _api_key.is_empty():
		_busy = false
		push_error("OpenAI API key not set. Please configure the API key in the main tab and spawn a new assistant.")
		return false
...
func read_models_response(body: PackedByteArray) -> Array[String]:
	...
	if response.has("data") and response.data is Array:
		...
		push_error("Failed to get model list from OpenAI: %s" % JSON.stringify(response))
		return [INVALID_RESPONSE]
...
func send_chat_request(http_request: HTTPRequest, content: Array) -> bool:
	...
	if error != OK:
		_busy = false
		push_error("OpenAI API chat request failed.\nURL: %s\nRequest body: %s" % [_chat_url, body])
		return false
	return true
...
func read_response(body: PackedByteArray) -> String:
	...
	if response.has("choices") and response.choices.size() > 0:
		...
		if choice.has("message") and choice.message.has("content"):
			return ResponseCleaner.clean(choice.message.content)
	
	push_error("Failed to parse OpenAI response: %s" % JSON.stringify(response))
	return INVALID_RESPONSE
