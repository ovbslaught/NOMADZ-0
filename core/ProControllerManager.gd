...
func _load_default_mappings() -> void:
	...
	axis_mappings = {
		"move_h": ControllerAxis.LX,
		"move_v": ControllerAxis.LY,
		"look_h": ControllerAxis.RX,
		"look_v": ControllerAxis.RY,
		"trigger_l": ControllerAxis.L2,
		"trigger_r": ControllerAxis.R2,
	}
...
func _register_controller(device_id: int) -> void:
	...
	rumble_supported[device_id] = OS.has_feature("gamepad_rumble") or (name.to_lower().contains("pro") or name.to_lower().contains("switch") or name.to_lower().contains("xbox"))
...
func _get_axis_name(axis: int) -> String:
	match axis:
		ControllerAxis.LX: return "lx"
		ControllerAxis.LY: return "ly"
		ControllerAxis.RX: return "rx"
		ControllerAxis.RY: return "ry"
		ControllerAxis.L2: return "l2"
		ControllerAxis.R2: return "r2"
		_: return ""
...
func get_controller_input(action: String, device_id: int = -1) -> float:
	...
	if action in ["move_left", "move_right", "move_forward", "move_back"]:
		var axis := ControllerAxis.LX if action in ["move_left", "move_right"] else ControllerAxis.LY
		var val := Input.get_joy_axis(device_id, axis as JoyAxis)
