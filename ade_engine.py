#!/usr/bin/env python3
"""
ADE-CORE Production Engine v1.1
Samsung Galaxy S23 Ultra | Termux Python 3.14+ | ARM64 Android
CLI: ade {init,synth,run,audit,status,sync,checkpoint}
"""
...
def pin_to_big_cores() -> None:
    try:
        ...
        log.warning("CPU pin skipped (non-fatal): %s", e)
...
CREATE TABLE IF NOT EXISTS ade_state_vectors(
    ...
    last_run_ts REAL NOT NULL DEFAULT (unixepoch('now')),
    heartbeat_node TEXT NOT NULL DEFAULT 'ADE-CORE',
...
def execute_skill(skill_id: int, name: str, entrypoint: str,
    ...
    t0 = time.monotonic(); res: dict[str, Any] = {"exit_code": -1, "duration_ms": 0, "output": ""}
    try:
        p = subprocess.run([bin_, "-c", entrypoint],
                           capture_output=True, text=True, timeout=60)
        ms = int((time.monotonic()-t0)*1000)
        res = {"exit_code": p.returncode, "duration_ms": ms, "output": p.stdout.strip()}
        log.info("Skill '%s' done (%dms exit=%d).", name, ms, p.returncode)
        if p.stdout.strip():
            try: print(p.stdout.strip())
            except Exception: pass
    except subprocess.TimeoutExpired:
        res["exit_code"] = 124; log.error("Skill '%s' timed out.", name)
    except Exception as exc:
        res["exit_code"] = 1; log.error("Skill '%s' error: %s", name, exc)
    ...
    with get_db() as con:
        con.execute("""
            ...
            VALUES(?,?,'execution',?,?,?,?)
        """, (skill_id, exec_id, res["exit_code"], res["duration_ms"], res["output"], time.time()))
    return res
...
def _record_error(skill_id: int, ec: str, tb: str) -> None:
    ...
    with get_db() as con:
        row = con.execute(
            "SELECT trace_id,occurrence_count FROM ade_error_traces WHERE fingerprint=?", (fp,)
        ).fetchone()
        if row:
            ...
            con.execute("""
                UPDATE ade_error_traces SET occurrence_count=?,quarantined=?,
                  ...
                  last_seen=unixepoch('now') WHERE fingerprint=?
            """, (cnt, q, q, fp))
            if q: log.warning("VULTURE: skill %d quarantined fp=%s", skill_id, fp)
        else:
            con.execute("""INSERT INTO ade_error_traces(skill_id,error_class,fingerprint,traceback)
...
def upsert_state_vector(skills_count: int = 0, open_errors: int = 0,
    ...
    with get_db() as con:
        ...
        """, (SESSION_ID,str(DB_PATH),str(ENGINE_ROOT),str(WORMHOLE_ROOT),
              skills_count,open_errors,time.time(),heartbeat_node,time.time()))

def vulture_brain_tick() -> None:
    e, s = open_error_count(), count_skills()
    upsert_state_vector(skills_count=s, open_errors=e, heartbeat_node="MOTHER-BRAIN-CORE")
...
def _handle_signal(sig: int, _: Any) -> None:
    global _SHUTDOWN
    log.info("Signal %d — graceful shutdown", sig); _SHUTDOWN = True
...
"
    "print(json.dumps({'node':'OMEGA-BRAIN','platform':platform.platform(),"
    "'arch':platform.machine(),'python':platform.python_version(),"
    "'pid':os.getpid(),'timestamp':int(time.time()),'status':'OPERATIONAL'}))"
)
...
def cmd_run(args: argparse.Namespace) -> None:
    ...
    with get_db() as con:
        row = con.execute(
            ...
            (name,)).fetchone()
    if not row:
        print(f"[ERROR] Skill '{name}' not found — run: ade synth --skill {name}", file=sys.stderr)
        sys.exit(1)
...
def cmd_status(_: argparse.Namespace) -> None:
    ...
    vec = {"session_id": SESSION_ID, "db_path": str(DB_PATH),
           "engine_root": str(ENGINE_ROOT), "wormhole": str(WORMHOLE_ROOT),
           "skills_count": s, "open_errors": e, "wal_mode": "wal",
...
def cmd_audit(_: argparse.Namespace) -> None:
    with get_db() as con:
        rows = con.execute("""
            SELECT log_id,skill_id,substr(execution_id,1,8) as execution_id,
                   event_type,exit_code,duration_ms,
                   datetime(created_at,'unixepoch') as ts
            FROM ade_runtime_logs ORDER BY log_id DESC LIMIT 10
        """).fetchall()
    print(f"{'log_id':>6} {'skill_id':>8} {'exec_id':>8} {'event':>12} "
          f"{'exit':>4} {'ms':>6}  ts")
    print("-"*64)
    for r in rows:
        print(f"{r['log_id']:>6} {r['skill_id']:>8} {r['execution_id']:>8} "
              f"{r['event_type']:>12} {r['exit_code']:>4} {r['duration_ms']:>6}  {r['ts']}")
...
def cmd_sync(_: argparse.Namespace) -> None:
    ...
    print(json.dumps({"status": status, "snapshot": str(dest)}, indent=2))

def main() -> None:
    p = argparse.ArgumentParser(prog="ade")
    p.add_argument("command",
        choices=["init","synth","run","audit","status","sync","checkpoint"])
    p.add_argument("--skill",   help="Skill name")
    p.add_argument("--entry",   help="Python code entrypoint string")
    p.add_argument("--runtime", help="Runtime binary (default: python3)")
    p.add_argument("--args",    nargs="...", help="Extra args to skill")
