...
# 1. Clean up any frozen processes
echo "[*] Clearing stalled processes..."
pkill -f "nomadz_agent.py" || true
...
mkdir -p "${BASE_DIR}/python_agent" "${BASE_DIR}/db" "${BASE_DIR}/logs"

# 3. Launch omega-core daemon with health verification
if [ -x "${BIN_DIR}/omega-core" ]; then
    echo "[+] Launching omega-core daemon on :8484..."
    nohup "${BIN_DIR}/omega-core" > "${BASE_DIR}/logs/omega_core.log" 2>&1 &
    sleep 1
fi

# 4. Deploy Non-Blocking Python Agent
cat << 'PYTHON_EOF' > "${AGENT_SCRIPT}"
#!/usr/bin/env python3
"""
NOMADZ-BRAIN: Ultra-Fast Non-Blocking Agentic Substrate
Zero-Wait Local Fallbacks, SQLite WAL Direct Concurrency, Fast ReAct Execution.
"""
...
# Set strict global socket timeout to prevent Termux hangs
...
def is_daemon_online() -> bool:
    try:
        with urllib.request.urlopen(f"{OMEGA_CORE_URL}/health", timeout=0.2) as resp:
            return resp.status == 200
    except Exception:
        return False

# ------------------------------------------------------------------------------
# 1. Omega Memory Manager (SQLite WAL)
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
# 2. Fast Non-Blocking LLM Gateway
# ------------------------------------------------------------------------------
class LLMGateway:
    def __init__(self):
    ...
    def query(self, prompt: str, system_prompt: str = "", history: str = "") -> str:
        ...
        # Instant Deterministic Recovery
    ...
    def _deterministic_step_planner(self, prompt: str, history: str) -> str:
        if "FLAG_TOPOLOGY_VERIFIED" not in history:
            cmd = f"ls -la '{WORMHOLE_ROOT}' && echo 'FLAG_TOPOLOGY_VERIFIED'"
            return (
                f"THOUGHT: Active LLM offline. Phase 1: Validating WORMHOLE root node hierarchy.\n"
                f"ACTION: execute_shell\n"
                f"ACTION_INPUT: {json.dumps({'command': cmd})}"
            )
        elif "FLAG_WAL_VERIFIED" not in history:
            cmd = f"python3 -c \"import sqlite3; conn=sqlite3.connect('{DB_PATH}'); print('WAL_STATUS:', conn.execute('PRAGMA journal_mode;').fetchone()[0])\" && echo 'FLAG_WAL_VERIFIED'"
            return (
                f"THOUGHT: Phase 2: Verifying SQLite WAL mode and memory schema.\n"
                f"ACTION: execute_shell\n"
                f"ACTION_INPUT: {json.dumps({'command': cmd})}"
            )
        elif "FLAG_SYS_VERIFIED" not in history:
            cmd = f"uname -a && df -h '{WORMHOLE_ROOT}' && echo 'FLAG_SYS_VERIFIED'"
            return (
                f"THOUGHT: Phase 3: Inspecting system resources and memory limits.\n"
                f"ACTION: execute_shell\n"
                f"ACTION_INPUT: {json.dumps({'command': cmd})}"
            )
        else:
            return (
                "THOUGHT: All substrate verification phases executed cleanly.\n"
                f"FINAL_ANSWER: Substrate online. WORMHOLE nodes intact at {WORMHOLE_ROOT}, SQLite WAL active at {DB_PATH}, and daemon telemetry logged successfully."
            )

# ------------------------------------------------------------------------------
# 3. Substrate Execution Matrix
# ------------------------------------------------------------------------------
class SubstrateTools:
    def __init__(self, memory: OmegaMemoryManager):
        self.memory = memory

    def execute_shell(self, command: str, working_dir: Optional[str] = None, timeout_sec: int = 30) -> Dict[str, Any]:
        if is_daemon_online():
            try:
                payload = {
                    "command": command,
...
class NomadzAgent:
    ...
    def _parse_action(self, response: str) -> Tuple[Optional[str], Dict[str, Any]]:
        action_match = re.search(r"ACTION:\s*([a-zA-Z0-9_-]+)", response)
        if not action_match:
            return None, {}
        tool_name = action_match.group(1).strip()
        
        input_start = response.find("ACTION_INPUT:")
        if input_start == -1:
            return tool_name, {}
        
        raw_input = response[input_start + len("ACTION_INPUT:"):].strip()
        json_match = re.search(r"(\{.*\})", raw_input, re.DOTALL)
        if json_match:
            try:
                return tool_name, json.loads(json_match.group(1))
            except Exception:
                pass
    ...
    def run(self, task: str, max_iterations: int = 10) -> str:
        ...
        print(f"[*] Task: '{task}'")
        print(f"[*] Substrate Node: [{primary_node}]")

        recalled = self.memory.recall_memory(task, limit=3)
...
Canonical WORMHOLE root: {WORMHOLE_ROOT}
        ...
        for iteration in range(1, max_iterations + 1):
            ...
            if "FINAL_ANSWER:" in response:
                ...
                self.memory.record_memory(
                    ...
                    content=f"Task '{task[:80]}' completed: {final_answer[:120]}",
            ...
            if tool_name:
                try:
                    ...
                    if tool_name == "execute_shell":
                        res = self.tools.execute_shell(tool_args.get("command", ""))
                        observation = f"Exit: {res['exit_code']}\nSTDOUT:\n{res['stdout']}\nSTDERR:\n{res['stderr']}"
                    elif tool_name == "read_file":
                        observation = self.tools.wormhole_read_file(tool_args.get("path", ""))
                    elif tool_name == "write_file":
                        observation = self.tools.wormhole_write_file(tool_args.get("path", ""), tool_args.get("content", ""))
                    elif tool_name == "recall_memory":
                        observation = json.dumps(self.memory.recall_memory(tool_args.get("query", "")))
                    elif tool_name == "record_memory":
                        mem_id = self.memory.record_memory(primary_node, tool_args.get("category", "GENERAL"), tool_args.get("content", ""))
