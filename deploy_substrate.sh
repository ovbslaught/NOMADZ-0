...
echo "[+] Initializing Substrate at: ${BASE_DIR}"
...
# ==============================================================================
# 2. RUST CORE: src/db.rs (SQLite WAL Manager)
# ==============================================================================
...
# ==============================================================================
# 3. RUST CORE: src/engine.rs (Subprocess Runner with Strict Timeouts)
# ==============================================================================
...
# ==============================================================================
# 4. RUST CORE: src/main.rs (Tokio Daemon & API Router)
# ==============================================================================
...
# ==============================================================================
# 5. PYTHON AGENT: nomadz_agent.py (Autonomous Agentic Brain & Substrate Router)
# ==============================================================================
...
# ------------------------------------------------------------------------------
# 1. Memory Store (Direct SQLite WAL Fallback & Rust Daemon Client)
# ------------------------------------------------------------------------------
class OmegaMemoryManager:
    ...
    def record_memory(self, node_source: str, category: str, content: str, metadata: dict = None) -> int:
        ...
        # Attempt via Rust daemon first, fallback to direct SQLite WAL
        try:
            req_data = json.dumps({
                "timestamp": now,
...
# ------------------------------------------------------------------------------
# 2. Multi-Provider LLM Gateway (Local Ollama, OpenRouter, Anthropic, Gemini)
# ------------------------------------------------------------------------------
class LLMGateway:
    def __init__(self):
    ...
    def query(self, prompt: str, system_prompt: str = "", model: Optional[str] = None) -> str:
        ...
        # 1. Try Local Ollama via Rust core or direct
        try:
            payload = {
                "model": target_model,
                "prompt": prompt,
        ...
        # 2. Fallback to OpenRouter / Gemini if key present
        ...
        # Deterministic self-contained fallback generator if offline
        return self._rule_based_fallback(prompt)

    def _rule_based_fallback(self, prompt: str) -> str:
        return (
            "THOUGHT: Local model offline and no cloud keys active. Operating in deterministic recovery mode.\n"
            "ACTION: execute_shell\n"
            "ACTION_INPUT: {\"command\": \"uname -a && uptime\"}"
        )

# ------------------------------------------------------------------------------
# 3. Substrate Tool Matrix
# ------------------------------------------------------------------------------
class SubstrateTools:
    ...
    def execute_shell(self, command: str, working_dir: Optional[str] = None, timeout_sec: int = 60) -> Dict[str, Any]:
        """Runs shell command with strict timeout via Rust Core Daemon or local subprocess."""
        # 1. Try Rust Daemon
        try:
            payload = {
                "command": command,
    ...
    def godot_build(self, project_path: str = "NOMADZ-0", export_preset: str = "Linux/X11") -> Dict[str, Any]:
        proj = os.path.join(WORMHOLE_ROOT, project_path)
        cmd = f"godot --headless --path '{proj}' --export-release '{export_preset}'"
        return self.execute_shell(cmd)
...
"""
        ...
        for iteration in range(1, max_iterations + 1):
            ...
            if "ACTION:" in response and "ACTION_INPUT:" in response:
                ...
                # If no formal action, conclude or summarize
                return response
...
if __name__ == "__main__":
    ...
    if args.daemon:
        ...
        default_task = "Verify WORMHOLE substrate integrity, check SQLite WAL status, and log node telemetry."
...
# ==============================================================================
# 6. BUILD & TEST SUBSTRATE
# ==============================================================================
echo "[+] Compiling Rust Core (omega-core)..."
...
echo "[+] Executing Python Agent Self-Test..."
